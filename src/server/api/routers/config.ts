...
export const configRouter = createTRPCRouter({
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    const pdfModules = existingBook.modules.map(p => {
      ...
      let pdfUrl = p.module.files.find(f => f.name?.startsWith("file_"))?.src ?? "/storage/notizen.pdf"
...
const checkout = await stripe.checkout.sessions.create(sessionParams as Stripe.Checkout.SessionCreateParams);
...
await db.payment.update({
  ...
  init: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const isUserConfig = existingBook?.createdById !== null && existingBook?.createdById !== ctx.session?.user.id
      ...
      return {
        ...
        tips: existingTips.map(tip => ({id: tip.id, title: tip.title, content: tip.tip}))
      };
    }),
});
