...
export async function getHolidays(
  input : {code?: string, start:Date, end: Date}
): Promise<DateItem[]> {
  const {code, start, end} = input
  const periodStart = formatDate(start)
  const periodEnd = formatDate(end)
  const holidaysLink = 
  `https://openholidaysapi.org/PublicHolidays?countryIsoCode=DE&validFrom=${periodStart}&validTo=${periodEnd}&languageIsoCode=DE&subdivisionCode=${code ?? "DE-SL"}`

  const schoolHolidaysLink = 
  `https://openholidaysapi.org/SchoolHolidays?countryIsoCode=DE&validFrom=${periodStart}&validTo=${periodEnd}&languageIsoCode=DE&subdivisionCode=${code ?? "DE-SL"}`
  ...
  try {
      ...
      for ( const day of data){
        const name = day.name.find(d => d.language === "DE")?.text ?? "Feiertag"
        const holidayItem = {
          id: day.id,
          name,
          date: day.startDate
